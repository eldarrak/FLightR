...
lig.raw$light<-exp(lig.raw$light)
lig.raw$datetime<-as.POSIXct(lig.raw$datetime, tz="UTC", format="%Y-%m-%dT%T")

str(lig.raw)
# so we have to create extract twilights
tw<-lig.raw[lig.raw$twilight>0 &  lig.raw$excluded==0 & lig.raw$datetime<as.POSIXct("2014-05-18", tz="UTC"),]
...
calib1 <- gl_twl[gl_twl$tFirst<=as.POSIXct("2013-08-05", tz="UTC"),] #designate first ten twilights for calibration
known.coord <- c(5.43, 52.93)
elev <- getElevation(calib1$tFirst,calib1$tSecond,calib1$type,known.coord,plot=T) #get sun elevation angle 

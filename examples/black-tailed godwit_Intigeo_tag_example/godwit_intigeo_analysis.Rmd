...
-->
# Bar tailed godwit analysis example with Intigeo tag
## Install package
```{r}
require(devtools)
...
```{r}
File="E391.csv"
lig.raw<-read.csv(File, stringsAsFactors =F)
``` 
Or you can download it straight into R:
```{r}
library(RCurl)
text <- getURL("https://raw.githubusercontent.com/eldarrak/FLightR/master/examples/
		black-tailed%20godwit_Intigeo_tag_example/E391.csv"
		, ssl.verifypeer = FALSE, followlocation = TRUE)
lig.raw<-read.csv(text=text, stringsAsFactors =F)
lig.raw$light<-exp(lig.raw$light) # this is needed because we log transformed data in convert.lux.to.tags()

```
Now we have data read and we want to process them with the `read.tags.light.twilight()` function:
```{r}
FLightR.data<-read.tags.light.twilight(lig.raw, start.date="2013-06-10", end.date="2014-05-17")
...
log.light.borders=c(2.5, 8) # default values for Intigeo tag
...
```{r}
Index<-which(Result$Spatial$Frid[,1]>(2))
```
...
Times.NL=find.times.distribution(Prob.of.being.in.NL,Result$Matrix.Index.Table$time)

...
The finction recognizes the data from all the common tag types: mk tags (produced by British Antarctic Survey, Lotek, and Migrate Technology Ltd.), Intigeo tags (Migrate technology Ltd.) and GDL tags (Swiss Ornithological Institute).
```{r, eval=FALSE, tidy=FALSE}
...
Geolocators measure light levels with different precision, and calibration is needed to establish the relationship between the measured and the expected light levels for each device. This relationship is depicted by the calibration parameters (slopes), calculated based on the data recorded in known (calibration) geographic positions, e. g. the locations where the animal was tagged, recaptured or observed. FLightR uses a 'template fit' observational model [Ekstrom 2004, 2007](#ekstrom_2004) to establish the linear (on a log-log scale) between the light irradiance measured in known locations with the irradiance expected there according to the astronomical position of the sun [Rakhimberdiev et al. 2015](#rakhimberdiev_movecol_2015).

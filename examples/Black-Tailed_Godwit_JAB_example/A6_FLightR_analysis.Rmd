...
log.irrad.borders=c(-4.5, 4.5) # default values for Intigeo tag
```
##Calibration
...
Proc.data<-process.twilights(FLightR.data$Data, 
	FLightR.data$twilights[FLightR.data$twilights$excluded==0,],
        measurement.period=Proc.data$measurement.period, saving.period=Proc.data$saving.period, impute.on.boundaries=Proc.data$impute.on.boundaries)
...
Result<-run.particle.filter(all.in, save.Res=F, cpus=min(Threads,6),
		...
		parallel=T,  plot=T, prefix="pf",
		extend.prefix=T, cluster.type="SOCK",
		a=45, b=1500, L=45, adaptive.resampling=0.99, check.outliers=F)
b= Sys.time()
b-a
...
```{r, eval = F}
if (nrow(Result$Results$Quantiles)==nrow(Result$Indices$Matrix.Index.Table)) {
      Quantiles=Result$Results$Quantiles
   } else {
      Quantiles<-Result$Results$Quantiles[-1,] 
   }

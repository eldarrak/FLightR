...
## Install package
We used FLightR 0.3.4 version, so you might want to install it if you want to get exactly the same results.

```{r, eval = F}
library(devtools)
install_github("eldarrak/FLightR@0.3.4") # note the version
library(FLightR)
```
...
setwd("your path")
convert.lux.to.tags("A2_raw_data.lux")
```
...
Many datasets contain some very bad data in the beginning and in the end. If you do not want to clean all these noisy entries by hand you may just write down start and end date of the data you want to work with and then use these values at the next step.

Let's now assume that you have done the previous step and have a [.csv](https://raw.githubusercontent.com/eldarrak/FLightR/master/examples/black-tailed_godwit_Intigeo_tag_example/A3_TAGS_format.csv) file downloaded from TAGS. Now you can process these data:

Note, there is also another workflow for twilight detection with BAStag package for [.lig files](https://github.com/eldarrak/FLightR/blob/master/examples/step_by_step_examples/lig%20-%20BAStags%20-%20FLightR%20workflow.Rmd) and [.lux files](https://github.com/eldarrak/FLightR/blob/master/examples/step_by_step_examples/lux%20-%20BAStags%20-%20FLightR%20workflow.Rmd).
```{r, eval = F}
...
```{r, eval = F}
library(RCurl)
text <- getURL("https://raw.githubusercontent.com/eldarrak/FLightR/master/examples/Black-Tailed_Godwit_JAB_example/A3_TAGS_format.csv",
           ssl.verifypeer = FALSE, followlocation = TRUE)
TAGS.twilights<-read.csv(text=text, stringsAsFactors =F)
TAGS.twilights$light<-exp(TAGS.twilights$light) 
# exp is needed because we log transformed data in convert.lux.to.tags()
```
...
log.light.borders=c(1.5, 8.5) # default values for Intigeo tag
log.irrad.borders=c(-4.25, 2.55) # default values for Intigeo tag
```
##Calibration

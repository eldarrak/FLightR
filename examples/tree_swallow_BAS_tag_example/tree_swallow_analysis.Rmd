...
version works wih FLightR 0.2.4
...
For now I will assume that one uses the TAGS service (http://tags.animalmigration.org) and saved light-twilight data from there. Below we download data in the TAGS outpur format
```{r}
library(RCurl)
...
model.ageing=T # set this FALSE is you are not going to model tag ageing.
	#Actually one can model it only f there are light data in known position in the beginning and in the end of the logger data

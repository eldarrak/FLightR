...
# ok, so now we want to plot data for two periods..
# and unfortunately we do not have s special functio for that..
# so from Calib.data.all we need preset of points..

#Calib.data<-Calib.data.all[Calib.data.all$Time<as.numeric(as.POSIXct("2011-07-13"))| Calib.data.all$Time>as.numeric(as.POSIXct("2012-04-11")),]

#str(Calib.data)

#plot(LogLight~LogIrrad, data=Calib.data, type="b")
#as.POSIXct("2012-04-11"))]
#as.POSIXct("2012-04-11"))
# ok, we have to try -7, 1.5
...
log.irrad.borders=c(-5.75,1.5)
calibration.parameters<-get.calibration.parameters(Calibration.periods, Proc.data, model.ageing=T, log.light.borders=log(c(2, 63)),  log.irrad.borders=log.irrad.borders)
...
FLightR.data$twilights$excluded[which(sapply(FLightR.data$twilights$datetime, FUN=function(x) min(abs(calib_outliers-as.numeric(x))))<3600)]<-1
...
Calibration=create.calibration(calibration.parameters$All.slopes, Proc.data, FLightR.data, log.light.borders=log(c(2,63)), log.irrad.borders=log.irrad.borders, start, ageing.model=calibration.parameters$ageing.model)
############################

library(parallel)
Threads=detectCores()-1

Outliers<-detect.tsoutliers(Calibration, Proc.data, plot=T, Threads=Threads, max.outlier.proportion=0.05, simple.version=F)
...
if (any(FLightR.data$twilights$datetime[FLightR.data$twilights$excluded==2]>=Calibration.start & FLightR.data$twilights$datetime[FLightR.data$twilights$excluded==2]<=Calibration.end)) {
#--------------------------------------------------------
# recalibration with outliers excluded..
#--------------------------------------------------------

#FLightR.data$twilights$excluded[which(FLightR.data$twilights$datetime <= as.POSIXct("2012-05-30"))]<-2
Proc.data<-process.twilights(FLightR.data$Data, FLightR.data$twilights[FLightR.data$twilights$excluded==0,], measurement.period=60, saving.period=120)
#---------------------------------
...
# this parts are needed to promote repeatability..

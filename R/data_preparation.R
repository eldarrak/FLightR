...
find.stationary.location<-function(Proc.data, calibration.start,  calibration.stop, plot=TRUE, initial.coords=NULL, print.optimization=TRUE, reltol=1e-4) {
  ...
  if (is.null(initial.coords)) stop('current function vesrion requires some inital coordinates to start search, they should not be very xclose but within few thousand km!')
	...
	if (length(calibration.parameters$calib_outliers)>0) {
      ...
      Proc.data_tmp, model.ageing=FALSE, 
	   ...
	   if (length(table(calibration.parameters$All.slopes$Slopes$Type))==1) {
		   ...
		   return(length(na.omit(calibration.parameters$All.slopes$Slopes$Type)))
		  } else {
          return(calibration.parameters$All.slopes$Parameters$LogSlope[2]^2-log(1/(percent_excluded+0.001)))
		  }
	   } else {
		   ...
		   print(table(calibration.parameters$All.slopes$Slopes$Type))
		   print('only_one_twilight_type_left!\n')
		   Val<-length(na.omit(calibration.parameters$All.slopes$Slopes$Type))
		   } else {
   ...
   cat('stage 1...\n')
   tryCatch(Res<-stats::optim(initial.coords, fn=ll_function, Proc.data=Proc.data, calibration.start=calibration.start, calibration.stop=calibration.stop, plot=plot, control=list(reltol=1e-2), method='BFGS'), finally=try(suppressWarnings(sink())))
   cat('stage 2...\n')

   tryCatch(Res<-stats::optim(Res$par, fn=ll_function, Proc.data=Proc.data, calibration.start=calibration.start, calibration.stop=calibration.stop, plot=plot, stage=2,control=list(reltol=reltol), method='BFGS' ), finally=try(suppressWarnings(sink())))
